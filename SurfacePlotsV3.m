...
ylabel('Distance to transition state (nm)')
...
set(gca,'XTick',-3:2,'XTickLabel',[0.001 0.01 0.1 1 10 100],'YTick',.1:.1:.3)
...
for ii=1:numel(x)
    ...
    fover40Shear(ii)=mean(CumStruct{1,ii}{1}>20);
    fover40Unzip(ii)=mean(CumStruct{2,ii}{1}>20);
    fover40T9(ii)=mean(CumStruct{3,ii}{1}>20);
