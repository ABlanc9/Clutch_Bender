...
function [mf,mv,mnb1,mnb2,mdint1,mdint2,mt,CumStruct,nOpenedTot,nPass] = clutchmodeltalinBender(nm,fm1,vu,nc,dint1,dint2,kont1,kont2,kof1,kof2,kc,ksub,konv,pt,mr,intadd,ion,fex,k_scale,xCut,xtst,Reinforce,Fcut)
...
% fex: Force extension curve - an n x 2 matrix where the first column is
%      extension and the second column is force. 
...
FcLast=Fc;
OnTime=zeros(nc,1); % Time since binding for each clutch
...
OnTimeCum=[];
nOpenedTot=0;
nPass=zeros(size(Fcut));

for t=0:ts:100


    % koff1 = kof1*koffcb(Fc,ion); % koff 1 in each clutch
    koff1=0.1863*exp(abs(Fc*10^12)*xtst/4.114)*k_scale;
    koff2 = 0.1863*exp(abs(Fc*10^12)*xtst/4.114);%kof2*koffcb(Fc,ion); %koff 2 in each clutch
    ...
    if t==0
        ...
        vf = vu.*(1 - abs(f(end))./Fs); % Filament speed
    end
    ...
    switch Reinforce
        ...
        case 2
            dint1 = dint1 + intadd.*sum((Fc>(60*10^-12)) & (FcLast<=(60*10^-12)));
            dint1 = dint1 - intadd.*sum((Fc<(60*10^-12)) & (FcLast>=(60*10^-12)));
    end
...
%     dint1
    ...
    nOpenedTot=nOpenedTot + sum((xc<xCut) & ((xc + vf.*ts.*boundbin)>=xCut));
    for i=1:length(Fcut)
        nPass(i)=nPass(i) + sum((FcLast<Fcut(i)) & (Fc >=Fcut(i)));
%         if (i==2) & (sum((FcLast<Fcut(i)) & (Fc >=Fcut(i))))
%             keyboard
%         end
    end

    xc = xc + vf.*ts.*boundbin; %Position of each clutch
    if sum(isnan(xc));keyboard;end
    xsub = 0; %Substrate position
    xc(bound == 0) = xsub;
    OnTime(bound == 0)=0;
    OnTime(boundbin)=OnTime(boundbin)+ts;
    FcLast=Fc;
    Fc=zeros(size(bound));
    Fc(boundbin)=interp1(fex(:,1),fex(:,2),abs(xc(boundbin))*10^9,'linear','extrap')*10^-12;
    Fc(Fc<0)=0;
    if sum(isnan(Fc));keyboard;end
    f(end +1) = sum(Fc);  % Force on substrate
    if f(end)<0;keyboard;end
    ...
    if t == 0;
        ...
        dint2t(1) = dint2i;
        OnTimeMn(1) = 0;
        OnTimeQ=zeros(1,length(Q));
    else;
        dint1t(end+1) = dint1;
        dint2t(end+1) = dint2;
        OnTimeMn(end+1) = mean(OnTime(boundbin));
        OnTimeQ(end+1,:)=quantile(OnTime(boundbin),Q);
%          OnTimeQ(end+1,:)=zeros(size(Q));
    end;
    % Fc = kc.*(xc - xsub); % Force in each clutch
    FcCum=[FcCum FcLast(boundbin)'*10^12];
    xCum=[xCum xc(boundbin)'*10^9];
    OnTimeCum=[OnTimeCum OnTime(boundbin)'];
    
end;

% figure(1)
subplot(2,3,1)
plot(ts*(1:length(f)),f)
hold on
xlabel('Time (s)')
ylabel('Force (N)')
subplot(2,3,2)
plot(ts*(1:length(f)),nb1)
hold on
xlabel('Time (s)')
ylabel('Number of Bound Clutches')
subplot(2,3,4)
histogram(FcCum,linspace(0,150,30),'EdgeColor','none')
hold on
xlabel('Force (pN)')
ylabel('Number of Bound Clutches')
subplot(2,3,5)
histogram(xCum,linspace(0,100,30),'EdgeColor','none')
hold on
xlabel('Extension (nm)')
ylabel('Number of Bound Clutches')
subplot(2,3,6)
histogram(OnTimeCum,linspace(0,.6,30),'EdgeColor','none')
hold on
xlabel('Bound Time (s)')
ylabel('Number of Bound Clutches')
...
mt = [mean(OnTimeMn,'omitnan') mean(OnTimeQ,'omitnan')];

CumStruct={FcCum(1:100:end),xCum(1:100:end),OnTimeCum(1:100:end)};

    FcCum=[FcCum Fc(boundbin)'*10^12];
    xCum=[xCum xc(boundbin)'*10^9];
    OnTimeCum=[OnTimeCum OnTime(boundbin)'];
